...
### 8.0: Quick Verification - Run This First

**Complete verification script to check all Phase 3 components.**

Save this as `databaseVerification.ps1` and run it anytime to validate your Phase 3 infrastructure:
...
**What This Checks:**
- ✅ All 3 resource groups exist
- ✅ All 3 SQL servers are deployed
- ✅ All 5 databases are online with correct status
- ✅ 3 admin security groups exist
- ✅ 3 app security groups exist
- ✅ 3 app registrations are created
- ✅ Audit logging is enabled on all servers
- ✅ Microsoft Defender is active
- ✅ Summary with clear pass/fail indicators

**Safe to run anytime** - 100% read-only operations, no modifications made.
...
echo "LAM Databases:"
az sql db list -g "db-lam-rg" -s "rhcdb-lam-sqlsvr" --query "[].{Name:name, Status:status, Edition:edition}" -o table

echo ""
echo "QA Databases:"
az sql db list -g "db-qa-rg" -s "rhcdb-qa-sqlsvr" --query "[].{Name:name, Status:status, Edition:edition}" -o table

echo ""
echo "Production Databases:"
az sql db list -g "db-prod-rg" -s "rhcdb-prod-sqlsvr" --query "[].{Name:name, Status:status, Edition:edition}" -o table
...
az sql server ad-only-auth get -g "db-lam-rg" -s "rhcdb-lam-sqlsvr" --query azureAdOnlyAuthentication
az sql server ad-only-auth get -g "db-qa-rg" -s "rhcdb-qa-sqlsvr" --query azureAdOnlyAuthentication
az sql server ad-only-auth get -g "db-prod-rg" -s "rhcdb-prod-sqlsvr" --query azureAdOnlyAuthentication
...
az sql server firewall-rule list -g "db-lam-rg" -s "rhcdb-lam-sqlsvr" -o table

echo ""
echo "QA Firewall Rules:"
az sql server firewall-rule list -g "db-qa-rg" -s "rhcdb-qa-sqlsvr" -o table

echo ""
echo "Production Firewall Rules:"
az sql server firewall-rule list -g "db-prod-rg" -s "rhcdb-prod-sqlsvr" -o table
